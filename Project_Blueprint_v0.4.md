...
- **Silver Layer (Validation & Dimension Extraction):**
    - **Jalur Fact Table:** Data kualitas air divalidasi menggunakan fungsi _native_ PySpark (cek _null rate_, duplikasi, tipe kolom, konsistensi skema). Baris yang gagal dicatat ke log dan dipindahkan ke `data/rejected/`.
...
**Minggu 2 — Silver Layer (Validation & Spatial Dimension):** Implementasi ingesti data (Bronze), dilanjutkan dengan validasi _native_ untuk data kualitas air (skema, _null rate_, duplikasi) dengan output ke `data/rejected/` untuk baris gagal. Ekstraksi tabel dimensi populasi (14 stasiun) via `rasterio`.
